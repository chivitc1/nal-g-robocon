...
class Target:
    ...
    def run(self):
        ...
        # instantiate images
        hsv_img = cv2.resize(cv2.getSize(cv2.QueryFrame(self.capture)), 8, 3)
        threshold_img1 = cv2.resize(cv2.getSize(hsv_img), 8, 1)
        threshold_img1a = cv2.resize(cv2.getSize(hsv_img), 8, 1)
        threshold_img2 = cv2.resize(cv2.getSize(hsv_img), 8, 1)
        i = 0
        writer = cv2.createVideoWriter('angle_tracking.avi', cv2.CV_FOURCC('M', 'J', 'P', 'G'), 30, cv2.getSize(
            hsv_img), 1)
        while True:
